...
const ModalComponent: React.FC<ModalProps> = ({ isOpen, onRequestClose }) => {
  ...
  return (
    <Modal
      ...
      className="justify-center absolute items-stretch bg-gray-50 flex flex-col px-8 rounded-2xl max-md:px-5"
    >
      ...
      <input
        type="text"
        className="border-[color:var(--color-light-light-3,#DFDFE6)] bg-gray-50 gap-2 mt-6 px-4 py-4 rounded-xl border-2 border-solid max-md:max-w-full max-md:flex-wrap"
        placeholder="Номер телефона"
        style={{ paddingLeft: '40px' }}
      />
      <div style={{ position: 'absolute', top: '67%', left: '13px', transform: 'translateY(-50%)' }}>
